## Hi there 👋 I'm Alex Ford Botelho
...
| **B3 Stock Data Pipeline & Analytics Platform** | End-to-end data pipeline for ingesting, processing, and analyzing Brazilian stock market (B3) data. Implements a Medallion Architecture (Landing → Bronze → Silver → Gold) with Spark and Delta Lake, supports local development with Docker & MinIO, cloud deployment on AWS (S3, Glue, Step Functions), and exposes processed market indicators through a FastAPI service for analytical and decision-making use cases. | Spark · Delta Lake · AWS S3 · AWS Glue · Step Functions · Docker · MinIO · FastAPI · yfinance · Terraform | [🔗](https://github.com/alexfordbotelho/postech-fiap-ml-tech-challege-2) |
| **Flight Delay Prediction** | End-to-end Data Science & Machine Learning pipeline using the 2015 U.S. domestic flights dataset (~5.8M rows). Includes EDA (9 visualizations), feature engineering (7+ variables), supervised models for delay classification (AUC-ROC 0.740) and delay regression (MAE ~30 min), plus unsupervised airport profiling with K-Means (K=4) and PCA (85% variance with 3 components) | Python · Pandas · scikit-learn · Plotly · Jupyter · ML (Classification/Regression) · K-Means · PCA | [🔗](https://github.com/alexfordbotelho/postech-fiap-ml-tech-challege-3) |
| **B3 Stock Price Forecasting Platform**|End-to-end Machine Learning platform for forecasting Brazilian stock closing prices using LSTM on historical B3 market data. Includes multi-asset time series preparation, per-ticker normalization to avoid cross-asset leakage, recursive multi-step forecasting, interactive dashboards for trend analysis and recommendation support, plus production-ready model serving and monitoring with API health checks and inference metrics.|Python · TensorFlow/Keras · Pandas · NumPy · FastAPI · Streamlit · Prometheus · Docker · Time Series Forecasting · LSTM|[🔗](https://github.com/alexfordbotelho/postech-fiap-ml-tech-challege-4)|
| **Adaptive Financial Offers Platform** | End-to-end MLOps platform for adaptive financial offers using Multi-Armed Bandits, real-time experimentation, feature flags, analytics, and an explainable RAG/LLM assistant. Runs locally with Docker or on Azure through Terraform. | FastAPI · Multi-Armed Bandits · Next.js · PostgreSQL · Kafka · MLflow · Azure OpenAI · Docker · Terraform | [🔗](https://github.com/alexfordbotelho/postech-fiap-ml-tech-challege-5-challenge) |
